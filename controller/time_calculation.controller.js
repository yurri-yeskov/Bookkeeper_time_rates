...
exports.findCustomer = (req, res) => { // Select a specified customer_id info
    ...
    client.query(query_str, function (err, result) {
        ...
        let new_arr = []; let new_customer = false;
        for (let i = 0; i < result.rows.length; i++) {
...
exports.calculationTime = (req, res) => { // calculcate Time for a special customer
    ...
    client.query(query_str, function (err, result) {
        ...
        for (let i = 0; i < result.rows.length; i++) {
            ...
            product_profile_nid_arr[product_profile_nid_arr.length] = result.rows[i].product_profile_nid;
            for (let j = customer_info_arr.length - 1; j >= 0; j--) {
                if (result.rows[i].package_id == customer_info_arr[j].package && result.rows[i].company_type == customer_info_arr[j].company_type) {
        ...
        n_product_profile_nid_arr = product_profile_nid_arr.filter(function(elem, index, self) {
            return index === self.indexOf(elem);
        });

        res.send({ calc_val: calculation_val, product_profile_nid: n_product_profile_nid_arr });
    });
};
...
exports.findTimeElememts = (req, res) => {
    ...
    client.query("SELECT * FROM task_manager.time_elements WHERE element_id <> 'A' ORDER BY element_id;", function (err, result) {
...
exports.addProductProfile = (req, res) => {
    ...
    let query_str = "INSERT INTO task_manager.product_profiles (product_profile_nid, use_a, note) VALUES ($1, $2, $3) RETURNING *; ";
...
exports.addProductProfilePackage = (req, res) => {
    ...
    let query_str = "INSERT INTO task_manager.product_profile_package (product_profile_id, package_id) VALUES ";
...
exports.updateProductProfilePackage = (req, res) => {
    ...
    let package_id_arr = JSON.parse(req.body.package_ids);
    let query_str = "DELETE FROM task_manager.product_profile_package WHERE product_profile_id=" + req.body.product_profile_id + ";" + 
...
exports.addProductProfileCompanyType = (req, res) => {
    ...
    let query_str = "INSERT INTO task_manager.product_profile_company_type (product_profile_id, company_type) VALUES ";
...
exports.updateProductProfileCompanyType = (req, res) => {
    ...
    let company_type_arr = JSON.parse(req.body.company_types);
    let query_str = "DELETE FROM task_manager.product_profile_company_type WHERE product_profile_id=" + req.body.product_profile_id + ";" + 
...
exports.addProductProfileTimeElement = (req, res) => {
    ...
    let query_str = "INSERT INTO task_manager.product_profile_time_element (product_profile_id, element_id, new_customer) VALUES ";
    query_str += "(" + req.body.product_profile_id + "," + element_id_arr[0] + ",'" + req.body.new_customer + "')";

    for (let i = 1; i < element_id_arr.length; i++) {
        query_str += ",(" + req.body.product_profile_id + "," + element_id_arr[i] + ",'" + req.body.new_customer + "')";
    }
...
exports.updateProductProfileTimeElement = (req, res) => {
    ...
    if (req.body.new_customer == 'FALSE')
        query_str = query_str + "DELETE FROM task_manager.product_profile_time_element WHERE product_profile_id=" + req.body.product_profile_id + ";";
    query_str = query_str + "INSERT INTO task_manager.product_profile_time_element (product_profile_id, element_id, new_customer) VALUES ";
    query_str += "(" + req.body.product_profile_id + "," + element_id_arr[0] + ",'" + req.body.new_customer + "')";
